...
#ifndef EXPLAIN_WAITPID_H
#define EXPLAIN_WAITPID_H

/**
  * The explain_syscall_waitpid function may be used to
  * interpret a waitpid "call" from the
  * command line, and then run it through
  * explain_syscall_errno_waitpid for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_waitpid had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_waitpid(int errnum, int argc, char **argv);

#endif /* EXPLAIN_WAITPID_H */
