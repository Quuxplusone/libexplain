...
#ifndef EXPLAIN_PUTC_H
#define EXPLAIN_PUTC_H

/**
  * The explain_syscall_putc function may be used to
  * interpret a putc "call" from the
  * command line, and then run it through
  * explain_syscall_errno_putc for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_putc had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_putc(int errnum, int argc, char **argv);

#endif /* EXPLAIN_PUTC_H */
