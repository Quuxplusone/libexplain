...
#ifndef EXPLAIN_PUTCHAR_H
#define EXPLAIN_PUTCHAR_H

/**
  * The explain_syscall_putchar function may be used to
  * interpret a putchar "call" from the
  * command line, and then run it through
  * explain_syscall_errno_putchar for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_putchar had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_putchar(int errnum, int argc, char **argv);

#endif /* EXPLAIN_PUTCHAR_H */
