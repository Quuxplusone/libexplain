...
#ifndef EXPLAIN_REALLOC_H
#define EXPLAIN_REALLOC_H

/**
  * The explain_syscall_realloc function may be used to
  * interpret a realloc "call" from the
  * command line, and then run it through
  * explain_syscall_errno_realloc for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_realloc had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_realloc(int errnum, int argc, char **argv);

/* vim:ts=8:sw=4:et */
#endif /* EXPLAIN_REALLOC_H */
