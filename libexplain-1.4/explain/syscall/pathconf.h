...
#ifndef EXPLAIN_PATHCONF_H
#define EXPLAIN_PATHCONF_H

/**
  * The explain_syscall_pathconf function may be used to
  * interpret a pathconf "call" from the
  * command line, and then run it through
  * explain_syscall_errno_pathconf for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_pathconf had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_pathconf(int errnum, int argc, char **argv);

#endif /* EXPLAIN_PATHCONF_H */
