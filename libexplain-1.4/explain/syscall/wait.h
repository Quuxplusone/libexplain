...
#ifndef EXPLAIN_WAIT_H
#define EXPLAIN_WAIT_H

/**
  * The explain_syscall_wait function may be used to
  * interpret a wait "call" from the
  * command line, and then run it through
  * explain_syscall_errno_wait for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_wait had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_wait(int errnum, int argc, char **argv);

#endif /* EXPLAIN_WAIT_H */
