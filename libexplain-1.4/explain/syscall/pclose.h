...
#ifndef EXPLAIN_PCLOSE_H
#define EXPLAIN_PCLOSE_H

/**
  * The explain_syscall_pclose function may be used to
  * interpret a pclose "call" from the
  * command line, and then run it through
  * explain_syscall_errno_pclose for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_pclose had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_pclose(int errnum, int argc, char **argv);

/* vim:ts=8:sw=4:et */
#endif /* EXPLAIN_PCLOSE_H */
