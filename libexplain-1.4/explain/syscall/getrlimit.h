...
#ifndef EXPLAIN_GETRLIMIT_H
#define EXPLAIN_GETRLIMIT_H

/**
  * The explain_syscall_getrlimit function may be used to
  * interpret a getrlimit "call" from the
  * command line, and then run it through
  * explain_syscall_errno_getrlimit for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_getrlimit had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_getrlimit(int errnum, int argc, char **argv);

#endif /* EXPLAIN_GETRLIMIT_H */
