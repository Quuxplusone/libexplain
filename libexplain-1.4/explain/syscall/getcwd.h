...
#ifndef EXPLAIN_GETCWD_H
#define EXPLAIN_GETCWD_H

/**
  * The explain_syscall_getcwd function may be used to
  * interpret a getcwd "call" from the
  * command line, and then run it through
  * explain_syscall_errno_getcwd for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_getcwd had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_getcwd(int errnum, int argc, char **argv);

#endif /* EXPLAIN_GETCWD_H */
