...
#ifndef EXPLAIN_PIPE_H
#define EXPLAIN_PIPE_H

/**
  * The explain_syscall_pipe function may be used to
  * interpret a pipe "call" from the
  * command line, and then run it through
  * explain_syscall_errno_pipe for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_pipe had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_pipe(int errnum, int argc, char **argv);

/* vim:ts=8:sw=4:et */
#endif /* EXPLAIN_PIPE_H */
