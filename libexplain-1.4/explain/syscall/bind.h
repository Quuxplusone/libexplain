...
#ifndef EXPLAIN_BIND_H
#define EXPLAIN_BIND_H

/**
  * The explain_syscall_bind function may be used to
  * interpret a bind "call" from the
  * command line, and then run it through
  * explain_syscall_errno_bind for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_bind had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_bind(int errnum, int argc, char **argv);

#endif /* EXPLAIN_BIND_H */
