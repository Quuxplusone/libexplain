...
#ifndef EXPLAIN_WAIT3_H
#define EXPLAIN_WAIT3_H

/**
  * The explain_syscall_wait3 function may be used to
  * interpret a wait3 "call" from the
  * command line, and then run it through
  * explain_syscall_errno_wait3 for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_wait3 had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_wait3(int errnum, int argc, char **argv);

#endif /* EXPLAIN_WAIT3_H */
