...
#ifndef EXPLAIN_REMOVE_H
#define EXPLAIN_REMOVE_H

/**
  * The explain_syscall_remove function may be used to
  * interpret a remove "call" from the
  * command line, and then run it through
  * explain_syscall_errno_remove for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_remove had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_remove(int errnum, int argc, char **argv);

#endif /* EXPLAIN_REMOVE_H */
