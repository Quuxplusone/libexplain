...
#ifndef EXPLAIN_EXECVE_H
#define EXPLAIN_EXECVE_H

/**
  * The explain_syscall_execve function may be used to
  * interpret a execve "call" from the
  * command line, and then run it through
  * explain_syscall_errno_execve for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_execve had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_execve(int errnum, int argc, char **argv);

#endif /* EXPLAIN_EXECVE_H */
