...
#ifndef EXPLAIN_FORK_H
#define EXPLAIN_FORK_H

/**
  * The explain_syscall_fork function may be used to
  * interpret a fork "call" from the
  * command line, and then run it through
  * explain_syscall_errno_fork for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_fork had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_fork(int errnum, int argc, char **argv);

#endif /* EXPLAIN_FORK_H */
