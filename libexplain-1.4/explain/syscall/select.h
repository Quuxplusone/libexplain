...
#ifndef EXPLAIN_SELECT_H
#define EXPLAIN_SELECT_H

/**
  * The explain_syscall_select function may be used to
  * interpret a select "call" from the
  * command line, and then run it through
  * explain_syscall_errno_select for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_select had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_select(int errnum, int argc, char **argv);

#endif /* EXPLAIN_SELECT_H */
