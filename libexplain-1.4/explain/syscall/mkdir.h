...
#ifndef EXPLAIN_MKDIR_H
#define EXPLAIN_MKDIR_H

/**
  * The explain_syscall_mkdir function may be used to
  * interpret a mkdir "call" from the
  * command line, and then run it through
  * explain_syscall_errno_mkdir for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_mkdir had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_mkdir(int errnum, int argc, char **argv);

#endif /* EXPLAIN_MKDIR_H */
