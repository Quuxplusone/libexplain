...
#ifndef EXPLAIN_GETCHAR_H
#define EXPLAIN_GETCHAR_H

/**
  * The explain_syscall_getchar function may be used to
  * interpret a getchar "call" from the
  * command line, and then run it through
  * explain_syscall_errno_getchar for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_getchar had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_getchar(int errnum, int argc, char **argv);

#endif /* EXPLAIN_GETCHAR_H */
