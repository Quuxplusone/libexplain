...
#ifndef EXPLAIN_CLOSEDIR_H
#define EXPLAIN_CLOSEDIR_H

/**
  * The explain_syscall_closedir function may be used to
  * interpret a closedir "call" from the
  * command line, and then run it through
  * explain_syscall_errno_closedir for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_closedir had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_closedir(int errnum, int argc, char **argv);

#endif /* EXPLAIN_CLOSEDIR_H */
