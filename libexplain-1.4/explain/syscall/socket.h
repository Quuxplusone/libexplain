...
#ifndef EXPLAIN_SOCKET_H
#define EXPLAIN_SOCKET_H

/**
  * The explain_syscall_socket function may be used to
  * interpret a socket "call" from the
  * command line, and then run it through
  * explain_syscall_errno_socket for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_socket had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_socket(int errnum, int argc, char **argv);

#endif /* EXPLAIN_SOCKET_H */
