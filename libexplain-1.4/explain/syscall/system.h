...
#ifndef EXPLAIN_SYSTEM_H
#define EXPLAIN_SYSTEM_H

/**
  * The explain_syscall_system function may be used to
  * interpret a system "call" from the
  * command line, and then run it through
  * explain_syscall_errno_system for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_system had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_system(int errnum, int argc, char **argv);

#endif /* EXPLAIN_SYSTEM_H */
