...
void
explain_syscall_pread(int errnum, int argc, char **argv)
{
    int             fildes;
    void            *data;
    size_t          data_size;
    ...
    if (argc != 4)
    {
        fprintf(stderr, "pread: requires 4 arguments, not %d\n", argc);
        exit(EXIT_FAILURE);
    }
    ...
    explain_wrap_and_print(stdout, explain_errno_pread(errnum, fildes, data,
        data_size, offset));
}
