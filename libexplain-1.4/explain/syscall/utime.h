...
#ifndef EXPLAIN_UTIME_H
#define EXPLAIN_UTIME_H

/**
  * The explain_syscall_utime function may be used to
  * interpret a utime "call" from the
  * command line, and then run it through
  * explain_syscall_errno_utime for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_utime had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_utime(int errnum, int argc, char **argv);

#endif /* EXPLAIN_UTIME_H */
