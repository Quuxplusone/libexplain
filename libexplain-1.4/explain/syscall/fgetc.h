...
#ifndef EXPLAIN_FGETC_H
#define EXPLAIN_FGETC_H

/**
  * The explain_syscall_fgetc function may be used to
  * interpret a fgetc "call" from the
  * command line, and then run it through
  * explain_syscall_errno_fgetc for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_fgetc had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_fgetc(int errnum, int argc, char **argv);

#endif /* EXPLAIN_FGETC_H */
