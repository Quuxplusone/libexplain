...
#ifndef EXPLAIN_ACCEPT_H
#define EXPLAIN_ACCEPT_H

/**
  * The explain_syscall_accept function may be used to
  * interpret a accept "call" from the
  * command line, and then run it through
  * explain_syscall_errno_accept for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_accept had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_accept(int errnum, int argc, char **argv);

#endif /* EXPLAIN_ACCEPT_H */
