...
#ifndef EXPLAIN_CONNECT_H
#define EXPLAIN_CONNECT_H

/**
  * The explain_syscall_connect function may be used to
  * interpret a connect "call" from the
  * command line, and then run it through
  * explain_syscall_errno_connect for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_connect had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_connect(int errnum, int argc, char **argv);

#endif /* EXPLAIN_CONNECT_H */
