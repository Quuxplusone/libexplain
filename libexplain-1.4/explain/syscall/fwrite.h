...
#ifndef EXPLAIN_FWRITE_H
#define EXPLAIN_FWRITE_H

/**
  * The explain_syscall_fwrite function may be used to
  * interpret a fwrite "call" from the
  * command line, and then run it through
  * explain_syscall_errno_fwrite for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_fwrite had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_fwrite(int errnum, int argc, char **argv);

#endif /* EXPLAIN_FWRITE_H */
