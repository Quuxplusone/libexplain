...
#ifndef EXPLAIN_GETC_H
#define EXPLAIN_GETC_H

/**
  * The explain_syscall_getc function may be used to
  * interpret a getc "call" from the
  * command line, and then run it through
  * explain_syscall_errno_getc for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_getc had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_getc(int errnum, int argc, char **argv);

#endif /* EXPLAIN_GETC_H */
