...
#ifndef EXPLAIN_FERROR_H
#define EXPLAIN_FERROR_H

/**
  * The explain_syscall_ferror function may be used to
  * interpret a ferror "call" from the
  * command line, and then run it through
  * explain_syscall_errno_ferror for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_ferror had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_ferror(int errnum, int argc, char **argv);

#endif /* EXPLAIN_FERROR_H */
