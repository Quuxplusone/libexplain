...
#ifndef EXPLAIN_WAIT4_H
#define EXPLAIN_WAIT4_H

/**
  * The explain_syscall_wait4 function may be used to
  * interpret a wait4 "call" from the
  * command line, and then run it through
  * explain_syscall_errno_wait4 for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_wait4 had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_wait4(int errnum, int argc, char **argv);

#endif /* EXPLAIN_WAIT4_H */
