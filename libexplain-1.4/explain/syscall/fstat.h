...
#ifndef EXPLAIN_FSTAT_H
#define EXPLAIN_FSTAT_H

/**
  * The explain_syscall_fstat function may be used to
  * interpret a fstat "call" from the
  * command line, and then run it through
  * explain_syscall_errno_fstat for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_fstat had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_fstat(int errnum, int argc, char **argv);

#endif /* EXPLAIN_FSTAT_H */
