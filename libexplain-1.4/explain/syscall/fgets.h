...
#ifndef EXPLAIN_FGETS_H
#define EXPLAIN_FGETS_H

/**
  * The explain_syscall_fgets function may be used to
  * interpret a fgets "call" from the
  * command line, and then run it through
  * explain_syscall_errno_fgets for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_fgets had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_fgets(int errnum, int argc, char **argv);

#endif /* EXPLAIN_FGETS_H */
