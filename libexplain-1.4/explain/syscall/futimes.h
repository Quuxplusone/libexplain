...
#ifndef EXPLAIN_FUTIMES_H
#define EXPLAIN_FUTIMES_H

/**
  * The explain_syscall_futimes function may be used to
  * interpret a futimes "call" from the
  * command line, and then run it through
  * explain_syscall_errno_futimes for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_futimes had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_futimes(int errnum, int argc, char **argv);

/* vim:ts=8:sw=4:et */
#endif /* EXPLAIN_FUTIMES_H */
