...
#ifndef EXPLAIN_FDOPEN_H
#define EXPLAIN_FDOPEN_H

/**
  * The explain_syscall_fdopen function may be used to
  * interpret a fdopen "call" from the
  * command line, and then run it through
  * explain_syscall_errno_fdopen for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_fdopen had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_fdopen(int errnum, int argc, char **argv);

#endif /* EXPLAIN_FDOPEN_H */
