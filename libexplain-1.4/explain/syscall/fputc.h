...
#ifndef EXPLAIN_FPUTC_H
#define EXPLAIN_FPUTC_H

/**
  * The explain_syscall_fputc function may be used to
  * interpret a fputc "call" from the
  * command line, and then run it through
  * explain_syscall_errno_fputc for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_fputc had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_fputc(int errnum, int argc, char **argv);

#endif /* EXPLAIN_FPUTC_H */
