...
#ifndef EXPLAIN_IOCTL_H
#define EXPLAIN_IOCTL_H

/**
  * The explain_syscall_ioctl function may be used to
  * interpret a ioctl "call" from the
  * command line, and then run it through
  * explain_syscall_errno_ioctl for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_ioctl had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_ioctl(int errnum, int argc, char **argv);

/* vim:ts=8:sw=4:et */
#endif /* EXPLAIN_IOCTL_H */
