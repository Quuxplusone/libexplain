...
#ifndef EXPLAIN_TRUNCATE_H
#define EXPLAIN_TRUNCATE_H

/**
  * The explain_syscall_truncate function may be used to
  * interpret a truncate "call" from the
  * command line, and then run it through
  * explain_syscall_errno_truncate for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_truncate had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_truncate(int errnum, int argc, char **argv);

#endif /* EXPLAIN_TRUNCATE_H */
