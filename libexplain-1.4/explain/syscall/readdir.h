...
#ifndef EXPLAIN_READDIR_H
#define EXPLAIN_READDIR_H

/**
  * The explain_syscall_readdir function may be used to
  * interpret a readdir "call" from the
  * command line, and then run it through
  * explain_syscall_errno_readdir for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_readdir had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_readdir(int errnum, int argc, char **argv);

#endif /* EXPLAIN_READDIR_H */
