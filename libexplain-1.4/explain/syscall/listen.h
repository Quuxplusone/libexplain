...
#ifndef EXPLAIN_LISTEN_H
#define EXPLAIN_LISTEN_H

/**
  * The explain_syscall_listen function may be used to
  * interpret a listen "call" from the
  * command line, and then run it through
  * explain_syscall_errno_listen for an explanation.
  *
  * @note
  * Because the event being explain happened in another
  * process, the results are not as good as if
  * explain_syscall_errno_listen had been called
  ...
  *    1:1 to the call arguments.
  */
void explain_syscall_listen(int errnum, int argc, char **argv);

#endif /* EXPLAIN_LISTEN_H */
