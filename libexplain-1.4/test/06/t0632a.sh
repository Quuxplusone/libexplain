...
TEST_SUBJECT="ioctl VIDIOCGWIN"
. test_prelude

test_config VIDIOCGCAP || pass

fmt > test.ok << 'fubar'
ioctl(fildes = 42, request = VIDIOCGWIN, struct video_window *data =
0xNNNNNNNN) failed, Invalid argument (EINVAL) because fildes is not
associated with an object to which ioctl VIDIOCGWIN can be applied
fubar
test $? -eq 0 || no_result

explain -e EINVAL ioctl 42 VIDIOCGWIN > test.out.4
test $? -eq 0 || fail
