...
TEST_SUBJECT="ioctl VIDIOCGFREQ"
. test_prelude

test_config VIDIOCGCAP || pass

fmt > test.ok << 'fubar'
ioctl(fildes = 42, request = VIDIOCGFREQ, unsigned long *data =
0xNNNNNNNN) failed, Invalid argument (EINVAL) because fildes is not
associated with an object to which ioctl VIDIOCGFREQ can be applied
fubar
test $? -eq 0 || no_result

explain -e EINVAL ioctl 42 VIDIOCGFREQ > test.out.4
test $? -eq 0 || fail
