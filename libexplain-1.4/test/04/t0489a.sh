...
TEST_SUBJECT="ustat EFAULT"
. test_prelude

cat > test.ok << 'fubar'
ustat(dev = MKDEV(0, 0), data = 0x09876543) failed, Bad address (EFAULT)
...
cat > test.ok2 << 'fubar'
ustat(dev = makedev(0, 0), data = 0x09876543) failed, Bad address (EFAULT)
because data refers to memory that is outside the process's accessible
address space; this is more likely to be a software error (a bug) than it
is to be a user error
fubar
...
explain -eEFAULT ustat 0 0x9876543 > test.out
test $? -eq 0 || fail
