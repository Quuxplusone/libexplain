...
TEST_SUBJECT="getsockopt EBADF"
. test_prelude

fmt > test.ok << 'fubar'
getsockopt(fildes = 42, level = SOL_SOCKET, name = SO_KEEPALIVE, data
= 0xNNNNNNNN, data_size = { 256 }) failed, Bad file descriptor (EBADF)
...
explain -e EBADF getsockopt 42 SOL_SOCKET SO_KEEPALIVE > test.out.4
test $? -eq 0 || fail
