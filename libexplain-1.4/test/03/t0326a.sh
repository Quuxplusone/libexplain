...
TEST_SUBJECT="accept ENOTSOCK"
. test_prelude

fmt > test.ok << 'fubar'
accept(fildes = 42, sock_addr = 0xNNNNNNNN, sock_addr_size = { XXX })
failed, Socket operation on non-socket (ENOTSOCK) because fildes does
not refer to a socket
fubar
test $? -eq 0 || no_result

explain -e ENOTSOCK accept 42 > test.out4
test $? -eq 0 || fail
