...
TEST_SUBJECT="fdopen ENOMEM"
. test_prelude

fmt > test.ok << 'fubar'
fdopen(fildes = 1, flags = "w") failed, Cannot allocate memory (ENOMEM)
...
explain -e ENOMEM fdopen 1 "w" > test.out4
test $? -eq 0 || fail
...
diff test.ok2 test.out >/dev/null 2>/dev/null && pass
