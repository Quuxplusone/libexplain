...
fmt > test.ok << 'fubar'
chown(pathname = "foobar", owner = 0, group = 0) failed, Operation not
permitted (EPERM) because the process effective UID nnn is the same as
the owner UID of pathname but this is not sufficient privilege to change
the owner UID, and the process is not privileged
fubar
