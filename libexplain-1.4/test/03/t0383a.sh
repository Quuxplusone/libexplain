...
TEST_SUBJECT="setsockopt EFAULT"
. test_prelude

fmt > test.ok << 'fubar'
setsockopt(fildes = 42, level = SOL_SOCKET, name = SO_KEEPALIVE, data =
0xNNNNNNNN, data_size = 4) failed, Bad address (EFAULT) because data
...
explain -e EFAULT setsockopt 42 SOL_SOCKET SO_KEEPALIVE > test.out.4
test $? -eq 0 || fail
