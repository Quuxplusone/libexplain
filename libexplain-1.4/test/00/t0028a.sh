...
fmt > test.ok.2 << fubar
link(oldpath = "foo", newpath = "bar") failed, No such file or
directory (ENOENT) because there is no "foo" regular file in
the oldpath "$testdir" directory
fubar
test $? -eq 0 || no_result

explain -e ENOENT link foo bar > test.out.b
test $? -eq 0 || fail
