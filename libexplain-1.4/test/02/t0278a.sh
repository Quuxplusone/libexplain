...
= NNN) failed, Permission denied (EACCES) because the process tried to
connect to a broadcast address without having the socket broadcast flag
enabled; or, the connection request failed because of a local firewall rule
fubar
test $? -eq 0 || no_result

explain -eEACCES connect 42 > test.out4
test $? -eq 0 || fail
