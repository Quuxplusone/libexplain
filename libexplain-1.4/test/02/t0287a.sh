...
TEST_SUBJECT="connect EINPROGRESS"
. test_prelude

fmt > test.ok << 'fubar'
connect(fildes = 42, serv_addr = { sa_family = PF_UNSPEC }, serv_addr_size
= NNN) failed, Operation now in progress (EINPROGRESS) because the socket
is non-blocking and the connection cannot be completed immediately;
this is more likely to be a software error (a bug) than it is to be a
user error
fubar
test $? -eq 0 || no_result

explain -eEINPROGRESS connect 42 > test.out4
test $? -eq 0 || fail
