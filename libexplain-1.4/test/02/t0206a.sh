...
TEST_SUBJECT="mkdir EFAULT"
. test_prelude

fmt > test.ok << 'fubar'
mkdir(pathname = 0xNNNNNNNN, mode = S_IRWXU | S_IRWXG | S_IRWXO) failed,
Bad address (EFAULT) because pathname refers to memory that is
...
explain -e EFAULT mkdir foobar > test.out4
test $? -eq 0 || fail
