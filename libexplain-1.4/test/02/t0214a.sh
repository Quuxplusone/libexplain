...
TEST_SUBJECT="mkdir EROFS"
. test_prelude

fmt > test.ok.1 << 'fubar'
mkdir(pathname = "foobar", mode = S_IRWXU | S_IRWXG | S_IRWXO) failed,
Read-only file system (EROFS) because write access was requested and
pathname refers to a file on a read-only file system ("/example",
42% full)
fubar
test $? -eq 0 || no_result
...
explain -e EROFS mkdir foobar > test.out4
test $? -eq 0 || fail
