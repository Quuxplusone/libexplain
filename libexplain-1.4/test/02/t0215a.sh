...
TEST_SUBJECT="path resolution EACCES"
. test_prelude
...
does not have write permission to the pathname "a/foo" directory, this is
needed to create the directory entry for the "bar" regular file
fubar
test $? -eq 0 || no_result
...
test_creat a/foo/bar > test.out 2>&1
if test $? -ne 1
then
    chmod u+w a/foo
    echo expect to fail
