...
TEST_SUBJECT="connect EINTR"
. test_prelude

fmt > test.ok << 'fubar'
connect(fildes = 42, serv_addr = { sa_family = PF_UNSPEC }, serv_addr_size
= NNN) failed, Interrupted system call (EINTR) because the process was
interrupted by a signal before the connect was complete
fubar
test $? -eq 0 || no_result

explain -eEINTR connect 42 > test.out4
test $? -eq 0 || fail
