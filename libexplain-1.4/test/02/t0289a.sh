...
TEST_SUBJECT="connect EISCONN"
. test_prelude

fmt > test.ok << 'fubar'
connect(fildes = 42, serv_addr = { sa_family = PF_UNSPEC }, serv_addr_size
= NNN) failed, Transport endpoint is already connected (EISCONN) because
the socket is already connected to a network address; this is more likely
to be a software error (a bug) than it is to be a user error
fubar
test $? -eq 0 || no_result

explain -eEISCONN connect 42 > test.out4
test $? -eq 0 || fail
