...
TEST_SUBJECT="select EBADF"
. test_prelude

fmt > test.ok << 'fubar'
select(nfds = 42, readfds = { 0 }, writefds = { 1 }, exceptfds = {},
timeout = { 60 seconds }) failed, Bad file descriptor (EBADF) because an
invalid file descriptor was given in one of the sets; perhaps a file
descriptor that was already closed, or one on which an error has occurred
fubar
test $? -eq 0 || no_result

explain -eEBADF select 42 > test.out4
test $? -eq 0 || fail
