...
TEST_SUBJECT="chmod EROFS"
. test_prelude

fmt > test.ok.1 << 'fubar'
chmod(pathname = "foobar", mode = S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)
failed, Read-only file system (EROFS) because write access was requested
and pathname refers to a file on a read-only file system ("/example",
42% full)
fubar
test $? -eq 0 || no_result
...
chmod(pathname = "foobar", mode = S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)
failed, Read-only file system (EROFS) because write access was requested
and pathname refers to a file on a read-only file system
fubar
test $? -eq 0 || no_result
...
explain -e EROFS chmod foobar > test.out4
test $? -eq 0 || fail
