...
TEST_SUBJECT="rmdir EROFS"
. test_prelude

fmt > test.ok.1 << 'fubar'
rmdir(pathname = "snot") failed, Read-only file system (EROFS) because
write access was requested and pathname refers to a file on a read-only
file system ("/example", 42% full); note that pathname still exists
fubar
test $? -eq 0 || no_result
...
rmdir(pathname = "snot") failed, Read-only file system (EROFS) because
write access was requested and pathname refers to a file on a read-only
file system; note that pathname still exists
fubar
test $? -eq 0 || no_result
...
explain -e EROFS rmdir snot > test.out4
test $? -eq 0 || fail
