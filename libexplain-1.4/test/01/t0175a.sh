...
TEST_SUBJECT="rmdir ENOTEMPTY"
. test_prelude

cat > test.ok << 'fubar'
rmdir(pathname = "a") failed, Directory not empty (ENOTEMPTY) because
pathname is not an empty directory; that is, it contains entries other than
"." and ".." (2); note that pathname still exists
fubar
test $? -eq 0 || no_result
